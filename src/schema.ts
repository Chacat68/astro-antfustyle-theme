...
/* Posts */
const createPostSchema = ({ image }: SchemaContext, titleMax: number) =>
  z
    .object({
      title: z
        .string()
      ...
      image: z
        .string()
        .default('')
        .describe('Legacy cover image URL retained for this customized site.'),
      ...
      pubDate: z.coerce
        ...
        .optional()
        .describe(
          'Specifies the publication date. Legacy `published` is also accepted.'
        ),
    ...
    .superRefine((data, ctx) => {
      if (!data.pubDate && data.published) data.pubDate = data.published
      if (!data.lastModDate && data.updated) data.lastModDate = data.updated
      if (!data.pubDate) {
        ctx.addIssue({
          code: 'custom',
          path: ['pubDate'],
...
// 英文内容沿用同一套 Astro 7 schema；标题长度由实际内容控制。
export const postSchema = (context: SchemaContext) =>
  createPostSchema(context, 60)

export const postSchemaEn = (context: SchemaContext) =>
  createPostSchema(context, 120)
